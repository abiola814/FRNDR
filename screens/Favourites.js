...
useEffect(() => {
  const getUser = () => {
    ...
    const unsub = onSnapshot(doc(db, "users", userdetails.user.uid),(documentSnapshot) => {
      // console.log("Current data: ", documentSnapshot.data());
      var fav = documentSnapshot.data().likes
      fav.map((dat)=>{

          const docRefs = onSnapshot(doc(db, "users", dat),(docum) => {
            
            setUserData(current => [...current,{id: docum.id,photo: docum.data()? docum.data().image1:"",name: docum.data().name,address: docum.data().location}])
          })
